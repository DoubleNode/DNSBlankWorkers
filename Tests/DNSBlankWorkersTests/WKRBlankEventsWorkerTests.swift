//
//  WKRBlankEventsWorkerTests.swift
...
final class WKRBlankEventsWorkerTests: XCTestCase {
    private var sut: WKRBlankEvents!
    ...
    func test_register_setsNextWorker() {
        let nextWorker = WKRBlankEvents()
        sut.register(nextWorker: nextWorker, for: .whenUnhandled)

        XCTAssertEqual(sut.callNextWhen, .whenUnhandled)
        XCTAssertNotNil(sut.nextWKRPTCLEvents)
        XCTAssertTrue(sut.nextWKRPTCLEvents === nextWorker)
    }

    // MARK: - Error Handling Tests
    func test_confirmFailureResult_withEventsNotFoundError_returnsNotFound() {
        let error = DNSError.Events.notFound(field: "testField", value: "TestError", DNSCodeLocation(self))
    ...
    func test_confirmFailureResult_withOtherError_returnsOriginalResult() {
        let error = DNSError.Events.unknown(DNSCodeLocation(self))
        let result = sut.confirmFailureResult(.failure(error), with: error)
        switch result {
        case .failure:
            XCTAssertTrue(true)
