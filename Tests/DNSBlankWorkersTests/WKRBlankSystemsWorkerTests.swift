//
//  WKRBlankSystemsWorkerTests.swift
...
final class WKRBlankSystemsWorkerTests: XCTestCase {
    private var sut: WKRBlankSystems!
    ...
    func test_register_setsNextWorker() {
        let nextWorker = WKRBlankSystems()
        sut.register(nextWorker: nextWorker, for: .whenUnhandled)

        XCTAssertEqual(sut.callNextWhen, .whenUnhandled)
        XCTAssertNotNil(sut.nextWKRPTCLSystems)
        XCTAssertTrue(sut.nextWKRPTCLSystems === nextWorker)
    }

    // MARK: - Error Handling Tests
    func test_confirmFailureResult_withSystemsNotFoundError_returnsNotFound() {
        let error = DNSError.Systems.notFound(field: "testField", value: "TestError", DNSCodeLocation(self))
    ...
    func test_confirmFailureResult_withOtherError_returnsOriginalResult() {
        let error = DNSError.Systems.unknown(DNSCodeLocation(self))
        let result = sut.confirmFailureResult(.failure(error), with: error)
        switch result {
        case .failure:
            XCTAssertTrue(true)
