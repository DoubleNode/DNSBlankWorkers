//
//  WKRBlankMediaWorkerTests.swift
...
final class WKRBlankMediaWorkerTests: XCTestCase {
    private var sut: WKRBlankMedia!
    ...
    func test_confirmFailureResult_withMediaNotFoundError_returnsNotFound() {
        let error = DNSError.Media.notFound(field: "testField", value: "TestError", DNSCodeLocation(self))
    ...
    func test_confirmFailureResult_withOtherError_returnsOriginalResult() {
        let error = DNSError.Media.unknown(DNSCodeLocation(self))
        let result = sut.confirmFailureResult(.failure(error), with: error)
        switch result {
        case .failure:
            XCTAssertTrue(true)
